...
export default function Page({ initialClanInfo }: { initialClanInfo: any }) {
  ...
  useEffect(() => {
    if (!initialClanInfo) return;

    setclanInfo(initialClanInfo);

    if (initialClanInfo.name) {
      fetch(
        `/api/player-info?userId=${initialClanInfo.owner}&fields=username,displayName`,
      )
        .then((res) => {
          ...
          <div className="md:mt-24 flex flex-col lg:flex-row gap-6 w-full max-w-screen-xl px-4">
            <div className="bg-stone-800 select-none opacity-0 w-full lg:w-2/3 border border-stone-700 rounded-md p-4 flex flex-col sm:flex-row gap-6 animate-fade-in-first">
              <div className="md:mt-[-120px] bg-stone-800 overflow-hidden p-3 rounded-md w-fit">
                <GroupIcon clanId={initialClanInfo.id} />
              </div>
              <div className="flex flex-col gap-2 justify-between">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  ...
                  <Tooltip
                    text="The date which this clan was created."
                    position="bottom"
                  >
                    <div className="hover:cursor-pointer text-lg lg:text-2xl flex gap-2 text-stone-300 bg-stone-900 rounded-md p-2 px-4 items-center font-bold">
                      ...
                      <p>
                        {clanInfo.createdAt
                          ? new Date(clanInfo.createdAt).toLocaleDateString(
                              "en-US",
                              {
                                year: "numeric",
                                month: "long",
